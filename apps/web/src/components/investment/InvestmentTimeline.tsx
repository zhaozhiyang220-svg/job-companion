...
export function InvestmentTimeline({ appId }: { appId: string }) {
  ...
  if (!data?.length) return <p className="text-sm text-fg-subtle">{t('empty')}</p>
      ...
      {data.map((iv) => {
        ...
        return (
          <li key={iv.id} className="relative">
            <span className="absolute -left-[1.55rem] top-0.5 flex h-5 w-5 items-center justify-center bg-fg text-fg-inverse">
            ...
            <div className="flex items-center gap-2 text-sm text-fg-muted">
              ...
              {iv.used_branch_label && (
                <span className="bg-bg-muted px-1.5 font-mono text-xs">
                  {iv.used_branch_label}
                </span>
              )}
            </div>
            {iv.notes && <p className="mt-1 text-sm text-fg-muted">{iv.notes}</p>}
          </li>
        )
      })}
    </ol>
